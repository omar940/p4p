...
#    ECF_threshold = 2.326*ECF_std+ECF_mean #99th percentile
    ...
    max_label = 'Maximum (<10%)'
    target_label = 'Met Target (<50%)'
    thresh_label = 'Met Threshold (<95%)'
    outlier_label = 'Outliers (>95%)'
    #assign P4P points
    for i in range(1,n+1):
        if df.loc[i,'pred100']<=ECF_maxpts:
            df.loc[i,'P4P_bench']=max_P4P
            df.loc[i,'Category']=max_label
        elif df.loc[i,'pred100']<=ECF_target and df.loc[i,'pred100']>ECF_maxpts:
            df.loc[i,'P4P_bench']=round(df.loc[i,'pred100']*m1+b1,2)
            df.loc[i,'Category']=target_label
        elif df.loc[i,'pred100']<=ECF_threshold and df.loc[i,'pred100']>ECF_target:
            df.loc[i,'P4P_bench']=round(df.loc[i,'pred100']*m2+b2,2)
...
#    labels = [max_label+str(counts.loc[max_label])+' Sites', target_label+str(counts.loc[target_label])+' Sites', thresh_label+str(counts.loc[thresh_label])+' Sites', outlier_label+str(counts.loc[outlier_label])+' Sites']
    ...
    #ax6.scatter(df.loc[:,'pred100'], df.loc[:,'P4P_bench'], label='Actual P4P Points', color='red', linewidths=0.5, edgecolors='black', zorder=2, s=10)
    y_mark = [max_P4P, y_1[-1], y_2[-1]]
    marker_labels = [max_label, target_label, thresh_label]
    ax6.scatter(benchmarks, y_mark, color='red', linewidths=0.5, marker='*',edgecolors='black', zorder=2, s=300)
    for k in range(len(benchmarks)):
        ax6.annotate(marker_labels[k], (benchmarks[k]+0.3, y_mark[k]+0.1))
    
    plt.ylim([-.25, max_P4P+0.3])
