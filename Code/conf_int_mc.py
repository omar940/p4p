...
def conf_int_mc(df, n, collab_mean_14):
    ...
    for i in range(1,n+1):
        lower_error = np.array([df.loc[i,'pred100']-df.loc[i,'lclm95']])
        upper_error = np.array([df.loc[i,'uclm95']-df.loc[i,'pred100']])
        ...
        if df.loc[i,'uclm95'] < collab_mean_14:
            if i<=samps:
                better = ax1.errorbar(df.loc[i,'pred100'], df.index[i-1], xerr=asymm_error, color='lightgreen', fmt='o', label='Better than Collaborative', elinewidth=3)
            df.loc[i,'P4P']=7.5
        ...
        elif df.loc[i,'lclm95'] > collab_mean_14:
            if i<=samps:
                worse = ax1.errorbar(df.loc[i,'pred100'], df.index[i-1], xerr=asymm_error, color='orange', fmt='o', label='Worse than Collaborative', elinewidth=3)
            df.loc[i,'P4P']=0
        #otherwise place in middle category with 5 points
        else:
            if i<=samps:
                middle = ax1.errorbar(df.loc[i,'pred100'], df.index[i-1], xerr=asymm_error, color='tan', fmt='o', label='Average', elinewidth=3)
            df.loc[i,'P4P']=5
...
#    #https://stackoverflow.com/questions/24988448/how-to-draw-vertical-lines-on-a-given-plot-in-matplotlib
    ...
    if 'worse' in locals():
        ...
        ax1.legend(handles=[collab_line, collab_line1, better, middle], loc=0)
    
        
                            
    #%%plot bar chart (distribution of providers)
    v = list(df.loc[:,'P4P'])
    counts = pd.value_counts(df['P4P'].values, ascending=True)
    x_pos = [0, 5, 7.5]
    labels = ['0', '5', '7.5']
