...
def best_fit_distribution(data, bins=20, ax=None):
    ...
    DISTRIBUTIONS = [        
        ...
        st.logistic,st.maxwell,st.mielke,st.nakagami,st.ncx2,st.nct,
        st.ncf,st.norm,st.powerlaw,st.powerlognorm,
        st.rayleigh,st.rice,st.recipinvgauss,st.semicircular,st.truncexpon,st.truncnorm,
...
def load_data(data, col_str):
    ...
    ax = data.plot(kind='hist', bins=20, density=True, alpha=0.5, edgecolor='black', linewidth=1.2) #color=plt.rcParams['axes.color_cycle'][1]
    # Save plot limits
    ...
    ax.set_xlabel(col_str+ u' ECF/Rehab (%)')
    ...
    data.plot(kind='hist', bins=20, density=True, alpha=0.5, label='Data', legend=True, ax=ax, edgecolor='black', linewidth=1.2)
    ...
    ax.set_xlabel(col_str+u' ECF/Rehab (% of patients) ')
    ax.set_ylabel('Frequency')
    
    dist_samples = gen_rand(best_dist, best_fir_paramms)
    #pd.Series(dist_samples).plot(kind='hist', bins=20, density=True, alpha=0.5, label='Monte Carlo', legend=True, ax=ax, edgecolor='black', linewidth=1.2)
...
#%%calculate benchmarks (max,target,threshold)
def benchmarks_calc(dist, params):
    ...
    #generate benchmarks
    maxpts = dist.ppf(0.10, *arg, loc=loc, scale=scale)
    target = dist.ppf(0.50, *arg, loc=loc, scale=scale)
