...
def point_sys(df, n):
    ...
    #plot error bars
    df.loc[:,'P4P_point']=round(df.loc[:,'Site_Rate']*m+b,2)
    ax3.plot(df.loc[:,'Site_Rate'], df.loc[:,'P4P_point'], label='Linear Payout Curve', color='lightgreen', alpha=.5, linewidth=7.0, zorder=1)
    ...
    #ax3.scatter(df.loc[:,'Site_Rate'], df.loc[:,'P4P_point'], label='Actual P4P Points', c='red', linewidths=1.0, edgecolors='black', zorder=2)
