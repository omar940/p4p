...
def ranking_plot(df, n):
    ...
    ax1.set_title('Profiling of Sites based on ECF based on Ranking')
    ...
    for i in range(n):
        ...
        x_bar = 0
        if i >= top_categ:
            better = ax1.errorbar(df.iloc[i,0], df.index[i], xerr=x_bar, color='lightgreen', fmt='o', label='Better than Collaborative (top 20%)')
        elif i <= mid_categ:
            worse = ax1.errorbar(df.iloc[i,0], df.index[i], xerr=x_bar, color='red', fmt='o', label='Worse than Collaborative (bottom 20%)')
        else:
            middle = ax1.errorbar(df.iloc[i,0], df.index[i], xerr=x_bar, color='orange', fmt='o', label='Average (middle 60%)')
